    ...
    async def sync_roles(self):
        ...
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT * FROM rolesyncer WHERE `approved` = 1")
                    ...
                    sync_configs[key].append(
                        {
                            ...
                            "source_guild_role": int(row[3]),
                            "target_guild_role": int(row[5]),
                            "prefix": row[7],
                            "use_prefix": bool(row[6]),
                        }
                    )
        ...
        for key, value in sync_configs.items():
            ...
            for source_guild_dict in value:
                ...
                for source_member in source_guild_members_with_role:
                    ...
                    prefix = source_guild_dict["prefix"] if source_guild_dict["use_prefix"] else ""
                    target_member_name = (
                        target_member.nick
